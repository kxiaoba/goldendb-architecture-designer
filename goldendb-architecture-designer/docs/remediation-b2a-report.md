...
> 历史检查点：下文“下一检查点”和“未发布”描述的是B2a验收时状态，不是当前待办。身份分离后来在B2c实现，字符边界在B2b/B2d继续修复；当前进度与剩余事项以 `remediation-progress.md` 顶部及 `remediation-workloads.md` 为准。真实Excel/WPS客户端验收仍未完成。保留原始报告供追溯。
